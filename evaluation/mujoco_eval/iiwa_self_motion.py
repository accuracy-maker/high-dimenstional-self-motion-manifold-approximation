...
3. clustering as 1-D SMM curves
...
def approximate_smm(tgt_pos, tgt_R, fk_tol = 0.05):
    ...
    print(f"min position error: {position_errors.min():.6f} m")
    print(f"mean position error: {position_errors.mean():.6f} m")

    print(f"min orientation error: {orientation_errors.min():.6f} rad")
    print(f"mean orientation error: {orientation_errors.mean():.6f} rad")

    print(f"min combined error: {errors.min():.6f}")
    print(f"mean combined error: {errors.mean():.6f}")

    # filter bad samples
    qs_filtered = qs[errors < fk_tol]
    # print(f"len of filtered qs: {len(qs_filtered)}")


    # clustering
    labels, k, gap = cluster_torus(qs_filtered, eps = 1.2)
...
# data.qpos[:7] = np.array([1,1,1, 1,1,1,1])
# mujoco.mj_forward(model, data)
# p = data.site_xpos[site_id]
# print(f"position: {p}")
# R =  data.site_xmat[site_id].reshape(3, 3)
# qs_filtered, labels, k, gap = approximate_smm(p, R)
